...
<Tab title="Entra">
Azure AD (Entra ID) supports two OIDC configuration modes for W&B Server. Choose the configuration that matches your security requirements:
...
4. On the "Register an application" screen, configure the following:
   - **Name**: Enter a descriptive name (for example, "Weights and Biases application")
...
You should now have collected two essential values:
...
### Confidential client
Use this configuration if you need to specify a client secret, due to security or compliance requirements.
...
7. Create a client secret:
   ...
   - Add a description (for example, "wandb")
